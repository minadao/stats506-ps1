...
By the table above, Infant has the highest correlation between shells weight and
rings at $0.725$.
...
```{r}
# filter from the data frame and count the number of abalones which have a 
# viscera weight larger than their shell weight
...
We saw that the total number of observations before restricting is 262, and the 
total after restricting is 230.

Eliminating rows from the extracting data with reasonable rules:

#### d. The variable related to age

I want to exclude all minors under the age of 18 since they usually are not 
responsible for food expenditures in the house.
```{r}
...
foodExpd <- filter(foodState, (totalExpd >= 0) & (groceryExpd >= 0) 
...
The final number of observations after cleaning is $121$ observations. We eliminated $230 - 121 = 109$ observations.
...
```{r}
#' Function to identify the next input in the Collatz sequence
#'
...
```{r}
#' Function to find the Collatz sequence and its length
#'
#' @param n a positive integer
